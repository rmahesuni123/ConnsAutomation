...
public class BillPayPageSetUpAutoPay extends  Conns_BillPay_Page_SetUp_AutoPay {
	...
	public void connsLogin(String email, String password, WebPage webPage, SoftAssert softAssert) throws Exception {
		...
		log.info("*******************************" +password);
		//log.info("*******************************" +commonData.get("passwordId"));
		commonMethods.sendKeysbyXpath(webPageMap.get(Thread.currentThread().getId()), commonData.get("emailId"), email, softAssert);
		log.info("*******************************" +password);
		commonMethods.sendKeysbyXpath(webPageMap.get(Thread.currentThread().getId()), commonData.get("passwordId"), password, softAssert);
		commonMethods.clickElementbyXpath(webPageMap.get(Thread.currentThread().getId()), commonData.get("loginButtonId"),
				softAssert);
		webPage.sleep(4000);
		/*
	...
	public static void DropDownValueErrorMessageAssertion(WebPage webPage, String[][] SelectPaymentDateDropDownData,
		...
		softAssert.assertTrue((commonMethods.getTextbyXpath(webPage, SelectPaymentDateDropDownData[3][1], softAssert)).contains(
				...
				log.info("Value less than  minimum amount or payment due . Expected " + SelectPaymentDateDropDownData[4][1] + " Actual : "
