...
import com.etouch.connsTests.TestConnsCreditAppPage;
import com.etouch.taf.core.exception.PageException;
import com.etouch.taf.util.LogUtil;
import com.etouch.taf.webui.selenium.WebPage;

public class CreditAppPage extends TestConnsCreditAppPage {
	...
	public static void navigateToCreditAppPage(SoftAssert softAssert) throws Exception	
	{
		...
		if(CreditAppUrl == null)
		{
			...
			commonMethods.clickElementbyXpath(webPage, commonData.get("YesMoneyGetCreditTodayApplyNowLink"), softAssert);
		...
		/*if(webPage.getPageTitle().equalsIgnoreCase(commonData.get("CreditAppPageTitle")))
		{log.info("Already on Credit App Page");
		webPage.getDriver().navigate().refresh();
		}
		else{
			if(!webPage.getPageTitle().equalsIgnoreCase(commonData.get("YesMoneyGetCreditTodayApplyNowLink")))
			{
	...
	/*public static void verifyTextField(WebPage webPage, String[][] locator, SoftAssert softAssert) throws Exception
{
		...
		for(int i= 0; i<locators.length;i++)
		{
			try{
				...
				{log.info("Failed to verify TextBox \""+locators[i][0] + "\" With Xpath : "+locators[i][1]+
						" Expected value is : \""+locators[i][2]+"\" Displayed Value : \""+element.getAttribute("value")+"\"");
			...
			{
				log.info("Failed to verify TextBox : "+locators[i][0]+" With Xpath "+locators[i][1]+" Localized Message :"+e.getLocalizedMessage());
				softAssert.fail("Failed to verify TextBox : "+locators[i][0]+" With Xpath "+locators[i][1]+" Localized Message :"+e.getLocalizedMessage());
			}
		}
	}
		...
		for(int i= 0; i<locators.length;i++)
		{
			try{
				...
				if(locators[i][1].equalsIgnoreCase("textField"))
				{
					...
					else{log.info("TextBox \""+locators[i][0] + "\" is displayed with expected value : \""+locators[i][3]+"\"");}
				}
				else if(locators[i][1].equalsIgnoreCase("dropDown"))
				{
					String actual = getSelectedValueFromDropDownXpath(softAssert, locators[i][0], locators[i][2]).trim();
					if(!actual.equalsIgnoreCase(locators[i][3]))
					{
						softAssert.fail("DropDown Expected value : "+locators[i][3]+" Actual value : "+actual);}
						else{log.info("DropDown \""+locators[i][0] + "\" is displayed with expected value : \""+locators[i][3]+"\"");}
				}
				else{
					softAssert.fail("Unable to Identify field type Field Name : "+locators[i][0]+" Field Type : "+locators[i][1]+" Xpath :"+locators[i][2]);
				}
				//webPage.getDriver().findElement(By.xpath(locators[i][1]));
				/*if(!element.getAttribute("value").equalsIgnoreCase(locators[i][2]))
				{log.info("Failed to verify TextBox \""+locators[i][0] + "\" With Xpath : "+locators[i][1]+
	...
	public static void submitCreditAppForNewUser(SoftAssert softAssert) throws Exception
	{
		...
		softAssert.assertTrue(actual
				.equals(commonData.get("NewUserSuccessPageMessage")),
	...
	public static String getSelectedValueFromDropDownID(SoftAssert softAssert,
	...
	public static String getSelectedValueFromDropDownXpath(SoftAssert softAssert,
			String FieldName, String locator)
	{
 ...
 */
	...
	public static void verifyTextFieldIsEditableByID(SoftAssert softAssert,String FieldName, String locator, String newValue)
	{
	...
	public static boolean verifyTextFieldIsEditableByXpath(SoftAssert softAssert,String FieldName, String locator, String newValue)
	{
		if(commonMethods.getWebElementbyXpath(webPage, locator, softAssert).isEnabled())
		{
			...
			if(!((String)js.executeScript("return arguments[0].getValue()", commonMethods.getWebElementbyXpath(webPage, locator, softAssert))==null))
			commonMethods.clearTextBoxByXpath(webPage, locator, softAssert);
			//WebElement element = commonMethods.getWebElementbyXpath(webPage, locator, softAssert);
			commonMethods.sendKeysbyXpath(webPage, locator, newValue, softAssert);
			
			String actual =(String)js.executeScript("return arguments[0].getValue()", commonMethods.getWebElementbyXpath(webPage, locator, softAssert));
			//	String actual = commonMethods.getTextbyXpath(webPage, locator, softAssert);
	...
	public static void verifyDropDownFieldIsEditableById(SoftAssert softAssert,String FieldName, String locator, String newValue)
	{
		if(commonMethods.getWebElementbyID(webPage, locator, softAssert).isEnabled())
		{
	...
	public static void verifyDropDownFieldIsEditableByXpath(SoftAssert softAssert,String FieldName, String locator, String newValue)
	{
		if(commonMethods.getWebElementbyXpath(webPage, locator, softAssert).isEnabled())
		{
			log.info("DropDown is enabled");
	...
	public static void selectValueFromDropDownByXpath(SoftAssert softAssert,String FieldName, String locator, String newValue)
	{
		log.info("Setting DropDown \""+FieldName+"\" Value to : "+newValue);
	...
	public static void fillForm(SoftAssert softAssert,String[][] FieldData)
	{
		...
		for(int i =0;i<dataLength;i++)
		{
			try{
				...
				case "dropDown":
					...
					//selectValueFromDropDownByXpath(softAssert, FieldData[i][0], FieldData[i][2], FieldData[i][3]);
					break;
				...
				default:
					softAssert.fail("Invalid Data in datasheet. FieldType is not set as expected. Current value is : "+FieldData[i][1]);
		...
		else if(FieldData[i][1].equalsIgnoreCase("radio"))
		{
			selectRadioButtonByXpath(softAssert, FieldData[i][0], FieldData[i][2]);
		}
		else{
			...
			{
				softAssert.fail("Failed to set value in "+FieldData[i][1]+"  \""+FieldData[i][0]+ "\" Due to :"+e.getLocalizedMessage());
			}
		}
	...
	public static WebElement getWebElementbyXpath(String locator, SoftAssert softAssert) throws Exception{
		...
		try{
			...
			throw new Exception("Unable to find element using Xpath : "+locator+". Localized Message: "+e.getLocalizedMessage());
		}
		return element;
	}
