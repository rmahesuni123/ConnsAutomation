	...
	public void verify_Yes_Lease_Page_MandatoryFiedlValidation() throws Exception {
		...
		if (declinedStatus == true) {
			try {
				...
				YesLeaseData = ExcelUtil.readExcelData(DataFilePath, "CreditApp",
						"verifyYesLeasePageData");
				for (int i = 0; i < 10; i++) {
					CreditAppPage.verifyErrorMessageByXpath(softAssert, YesLeaseData[i][0], YesLeaseData[i][1], YesLeaseData[i][2]);
				}
				log.info("testing verify_Yes_Lease_Page_MandatoryFiedlValidation completed------>");
				softAssert.assertAll();
			} catch (Throwable e) {
				mainPage.getScreenShotForFailure(webPage,
						"verify_Yes_Lease_Page_MandatoryFiedlValidation");
				softAssert.assertAll();
				Assert.fail(e.getLocalizedMessage());
			}
		}
	...
	public void verify_Yes_Lease_Page_Required_Date_Validation() throws Exception {
		...
		if (declinedStatus == true) {
			try {
				...
				//Verify for Hire Date with 31-Feb-2017 and 31-Dec-2018 and then today's Date	
				commonMethods.selectDropdownByValue(webPage, YesLeaseData[10][1],"02");
				commonMethods.selectDropdownByValue(webPage, YesLeaseData[11][1],"30");
				commonMethods.selectDropdownByValue(webPage, YesLeaseData[12][1],"2017");
				softAssert.assertEquals(webPage.findObjectByxPath(YesLeaseData[0][1]).getText(),YesLeaseData[0][2], "Hire Date:");
				
				commonMethods.selectDropdownByValue(webPage, YesLeaseData[10][1],"12");
				commonMethods.selectDropdownByValue(webPage, YesLeaseData[11][1],currentDate.substring(3, 5));
				commonMethods.selectDropdownByValue(webPage, YesLeaseData[12][1],currentDate.substring(6, 10));
				softAssert.assertEquals(webPage.findObjectByxPath(YesLeaseData[0][1]).getText(),YesLeaseData[0][2], "Hire Date:");
				
				commonMethods.selectDropdownByValue(webPage, YesLeaseData[10][1],currentDate.substring(0, 2));
				commonMethods.selectDropdownByValue(webPage, YesLeaseData[11][1],currentDate.substring(3, 5));
				commonMethods.selectDropdownByValue(webPage, YesLeaseData[12][1],currentDate.substring(6, 10));
				softAssert.assertFalse(webPage.findObjectByxPath(YesLeaseData[0][1]).getText().contains(YesLeaseData[0][2]), "Hire Date:");
				//Verify for Last Pay Date with current Date till last 35 days
				
				
				
				//Verify for Next Pay Date with current date till next 90 days
				
				
				//Verify for BANK Account Opened Date with 31-Feb-2017 and 31-Dec-2018 and then today's Date
				...
				mainPage.getScreenShotForFailure(webPage,
						"verify_Yes_Lease_Page_Required_Date_Validation");
				softAssert.assertAll();
				Assert.fail(e.getLocalizedMessage());
			}
		}
	...
	public void verify_Yes_Lease_Page_Payment_Details_Validatio() throws Exception {
		...
		if (declinedStatus == true) {
			try {
				...
				//Verify for Checking Account Number: This mandatory field will only accept numeric values no alphabets and special characters and numeric value count has to be between 4 and 17, not less than 4 or greater than 17
				...
				mainPage.getScreenShotForFailure(webPage,
						"verify_Yes_Lease_Page_Payment_Details_Validatio");
				softAssert.assertAll();
				Assert.fail(e.getLocalizedMessage());
			}
		}
	...
	public void verify_Yes_Lease_Page_Submition() throws Exception {
		...
		if (declinedStatus == true) {
			try {
		    	...
		    	//State could be declined-001, approved,-002, timeout-003,Invalid Unique ID-004, Wait-005
						
				log.info("testing verify_Yes_Lease_Page_Submition completed------>");
				softAssert.assertAll();
			} catch (Throwable e) {
				mainPage.getScreenShotForFailure(webPage,
						"verify_Yes_Lease_Page_Submition");
				softAssert.assertAll();
				Assert.fail(e.getLocalizedMessage());
			}
		}
