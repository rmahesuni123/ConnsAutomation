...
public class Conns_Money_Matters extends BaseTest {
	...
	public void Verify_Font_And_Size() {
		SoftAssert softAssert = new SoftAssert();
		try {
			String[][] ExpectedFontValuesWeb = ExcelUtil.readExcelData(DataFilePath, "MoneyMatters",
					"VerifyFontandSizeWeb");
			String[][] ExpectedFontValuesTab = ExcelUtil.readExcelData(DataFilePath, "MoneyMatters",
					"VerifyFontandSizeTab");
			String[][] ExpectedFontValuesMobile = ExcelUtil.readExcelData(DataFilePath, "MoneyMatters",
	...
	public void Verify_MoneyMatters_Text() {
		...
		try {
			...
			String[][] ExpectedValues = ExcelUtil.readExcelData(DataFilePath, "MoneyMatters", "VerifyFontandSize");
			System.out.println("Length : " + ExpectedValues.length);
			for (int i = 0; i < ExpectedValues.length; i++) {
				log.info("Value of i : " + i);
				actualTextValues = commonMethods.getTextbyXpath(webPage, ExpectedValues[i][1], softAssert);
				if (testType.equalsIgnoreCase("Mobile")) {
					softAssert.assertTrue(
							actualTextValues.toLowerCase().trim().contains((ExpectedValues[i][0]).toLowerCase().trim()),
							"Text value verification failed for element " + ExpectedValues[i][0] + "Expected Text : "
									+ ExpectedValues[i][0] + " Actual Text : " + actualTextValues);
				} else {
