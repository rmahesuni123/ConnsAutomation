...
public class Conns_Account_And_SignIn_Page extends BaseTest {
	...
	public void setUp(ITestContext context) throws InterruptedException, FileNotFoundException, IOException {
		try {
			...
			try {
				 ...
				 * getXlsDataConfig().get("testData")); DataFilePath =
	...
	public void verify_Login_Page_Title() {
		...
		try {
			...
			// "AccountSignINPage", "verifyRememberMeFunctionality");
			...
			softAssert.assertEquals(actualPageUrl, testdata[0][0],
					"Page url verification failed. Expected url : " + testdata[0][0] + "Actual url : " + actualPageUrl);
	 ...
	 * TafExcelDataProvider.class, priority = 302, enabled = true, description =
	 * "Verify FontandSize")
	 * 
	 * @ITafExcelDataProviderInputs(excelFile = "ConnsAccountSignINData",
	 * excelsheet = "AccountSignINPage", dataKey =
	 * "verifyFontandSizeOnLoginPage") public void
	 * verify_Font_and_Size_On_Login_Page(ITestContext context, TestParameters
	 * inputs) throws PageException, InterruptedException { Thread.sleep(2000);
	 * log.info(
	 * "**************Started Font and Size of content verification on Login Page *******************************"
	 * ); SoftAssert softAssert = new SoftAssert(); try{ int index = 0; try {
	 * List<String> ActualValues =
	 * ConnsSignInPage.getInfo(inputs.getParamMap().get("Locator"),
	 * inputs.getParamMap().get("IsAltPresent"),
	 * inputs.getParamMap().get("ObjType"),
	 * inputs.getParamMap().get("isTitlePresent")); List<String> exp_pageText =
	 * ConnsSignInPage.getExpectedInfo(inputs.getParamMap().get("TextAttributes"
	 * ), testBedName); ConnsSignInPage.verifyGivenValues(ActualValues,
	 * exp_pageText, "verifying font of" + index + "element"); } catch
	 * (Exception e) { log.error(
	 * ">--------------Test case verify font on login page failed -------------<"
	 * + e.getMessage()); SoftAssertor.addVerificationFailure(e.getMessage());
	 * e.printStackTrace(); } finally { index++; } log.info(
	 * "Verification of font and size completed on Login Page"); }
	 * catch(Throwable e){ mainPage.getScreenShotForFailure(webPage,
	 * "verifyLoginPageTitle"); softAssert.assertAll();
	 * Assert.fail(e.getLocalizedMessage()); } }
	 */
	/**
	 ...
	 * "This is a required field." should rendered on both the fields
	 * 
	 * @throws InterruptedException
	 */
	 ...
	 * view :Tap on Hamberger Menu -> 'SIGN IN' > "REGISTER button : Page should
	 * navigate to https://www.conns.com/customer/account/create/ Verify
	 * Redirectional links : Click On "click here" link under Registered
	 * Customers section is functional / Mobile view :Tap on Hamberger Menu ->
	 * 'SIGN IN' : https://www.conns.com/pay_your_bill should load without any
	 * error Verify Redirectional links : Click on "Forgot your Password" link /
	 * Mobile view :Tap on Hamberger Menu -> 'SIGN IN' : Page should navigate to
	 * https://www.conns.com/customer/account/forgotpassword/ and should load
	 * without any error Verify Redirectional links : Click on "Click here"
	 * under New customers section / Mobile view :Tap on Hamberger Menu -> 'SIGN
	 * IN' >'Click here' under New customers section : Page should navigate to
	 * https://www.conns.com/pay_your_bill and should load without any error
	 * 
	 * @throws InterruptedException
	 */
	...
	public void verify_Redirectional_links() throws InterruptedException {
		...
		webPage.getCurrentUrl();// For Safari
		/*
		 * try { window.navigate(Navigate_To_Account_Login_Form_URL); } catch
		...
		try {
			for (int r = 0; r < testdata.length; r++) {
				...
				js.executeScript("javascript: setTimeout(\"history.go(-1)\", 2000)");// Used
																						// for
																						// Safari
				// js.executeScript("javascript: setTimeout(\"history.go(0)\",
				// 2000)");
	 ...
	 * page should navigate to https://www.conns.com/customer/account/ And user
	 * name should displayed on top header account section(top right) Enter
	 ...
	 * -test123@mailinator.com password- Demo@1234
	 * 
	 * @throws InterruptedException
	 */
	...
	public void verify_Login_Functionality_Registered_User(ITestContext context) throws InterruptedException {
		...
		try {
			...
			// SoftAssertor.assertEquals(Actual_Element_Name,
			// Expected_Element_Name, "Element name does not match");
			String Actual_Page_Title = commonMethods.getPageTitle(webPage, softAssert);
			softAssert.assertEquals(Actual_Page_Title, Expected_Page_Title,
			...
			if (testType.equalsIgnoreCase("Mobile")) {
				...
				// ".//*[@id='li-primary-pronav-signout']/a");
				...
				// commonMethods.clickElementbyXpath(webPage, Locator_3,
				// softAssert);
				// commonMethods.clickElementbyXpath(webPage,
				// ".//*[@id='li-primary-pronav-signout']/a");
				// commonMethods.clickElementbyXpath(webPage, Locator_4,
				// softAssert);
			}
	 ...
	 *             Verify Page Title : Page should load without any error : Page
	 *             title should "Forgot Your Password" Navigate to
	 *             https://www.conns.com/customer/account/forgotpassword/ &
	 *             Mobile view :Tap on Hamberger Menu -> 'SIGN IN'-> Forgot Your
	 *             Password? link
	 */
	 ...
	 */
	/*
	 * @Test(priority = 312, enabled = true, description =
	 * "Verify_Font_And_Size") public void Verify_Font_And_Size(){ SoftAssert
	 * softAssert = new SoftAssert(); try { String[][] ExpectedFontValues =
	 * ExcelUtil.readExcelData(DataFilePath,
	 * "ConnsHomePageDev","VerifyFontandSize"); for(int
	 * i=0;i<ExpectedFontValues.length;i++) { // List<String> actualCssValues=
	 * commonMethods.getFontProperties(webPage, ExpectedFontValues[i][1]);
	 * 
	 * //
	 ...
	 * actualCssValues.get(1)); } softAssert.assertAll(); } catch(Throwable e){
	 * //mainPage.getScreenShotForFailure(webPage, "Verify_Font_And_Size");
	 * softAssert.assertAll(); Assert.fail(e.getLocalizedMessage()); } }
	 */
	/*
	 * @Test(dataProvider = "tafDataProvider", dataProviderClass =
	 * TafExcelDataProvider.class, priority = 312, enabled = true, description =
	 * "Verify FontandSize")
	 * 
	 * @ITafExcelDataProviderInputs(excelFile = "ConnsAccountSignINData",
	 * excelsheet = "AccountSignINPage", dataKey =
	 * "verifyFontandSizeOnForgotPasswordPage") public void
	 * verify_Font_and_Size_On_Forgot_Password_Page(ITestContext context,
	 * TestParameters inputs) throws PageException, InterruptedException {
	 * Thread.sleep(2000); log.info(
	 * "**************Started Font and Size of content verification on Forgot Password Page *******************************"
	 * ); int index = 0; try { List<String> ActualValues =
	 * ConnsSignInPage.getInfo(inputs.getParamMap().get("Locator"),
	 * inputs.getParamMap().get("IsAltPresent"),
	 * inputs.getParamMap().get("ObjType"),
	 * inputs.getParamMap().get("isTitlePresent")); List<String> exp_pageText =
	 ...
	 * link Page should load without any error
	 * 
	 * @throws InterruptedException
	 */
	...
	public void verify_Forgot_Password_Page_Submit_Button_Blank_Input() throws PageException {
		...
		try {
			webPage.getDriver().navigate().to(Account_Login_Page_Forgot_Password_Page_Link);
			webPage.getCurrentUrl();// For Safari
			// commonMethods.clickElementbyXpath(webPage, Forgot_Password_Link,
			// softAssert);
			commonMethods.clickElementbyXpath(webPage, Forgot_Password_Link, softAssert);
	...
	public void verify_Forgot_Password_Function_with_Invalid_Email_ID() throws PageException {
		...
		try {
			...
			// ConnsSignInPage.verify_Login_Functionality_with_Blank_Input(testdata);
			/*
			 * webPage.getDriver().navigate().back();
			 * webPage.getDriver().navigate().refresh();
			 */
			...
			// commonMethods.clickElementbyCssAndGetCurrentURL(webPage,
			// Forgot_Password_Link, softAssert);
			commonMethods.clickElementbyXpath(webPage, Forgot_Password_Link, softAssert);
	 ...
	 * 
	 * @throws PageException
	 *             Verify "Account Information" page title,Verify
	 *             "Account Information" form input validations by submitting
	 *             blank form,Verify email address field validation with some
	...
	public void verify_Account_Information_Tab_Verify_Validation_Messages() throws PageException, InterruptedException {
		...
		try {
			for (int r = 0; r < testdata.length; r++) {
				...
				if (FNInput.equalsIgnoreCase(testdata[1][2]) && LNInput.equalsIgnoreCase(testdata[1][5])
					...
					List<String> actualErrorMessage = ConnsSignInPage
							.verify_Account_Information_First_Name_Last_Name_Login_Invalid_Input_Validation(testdata,
									softAssert);
					log.info(
							"*************************************** Validation for Invalid Emails Starts: ******************************");
				...
				/*
				 * if (FNInput.equalsIgnoreCase(testdata[2][2]) &&
				 * LNInput.equalsIgnoreCase(testdata[2][5]) &&
				 * EmailInput.equalsIgnoreCase(testdata[2][8])) { log.info(
				 * "*************************************** Entering Valid Emails : ******************************"
				 * ); List<String> actualErrorMessage = ConnsSignInPage.
				 * verify_Account_Information_First_Name_Last_Name_Login_Valid_Input_Validation
				 * (testdata,softAssert); log.info(
				 * "*************************************** Validation for Valid Emails Starts: ******************************"
				 * ); softAssert.assertEquals(actualErrorMessage.get(0),
				 * Expected_Customer_Already_Exists_Error_Message,
				 * "Page Element Content verification failed. Expected_Email_ID_Error_Msg : "
				 * +Expected_Customer_Already_Exists_Error_Message +
				 * "actualErrorMessage.get(0) : "+actualErrorMessage.get(0));
				 * log.info(" actualErrorMessage : " +actualErrorMessage);
				 * log.info(" actualErrorMessage.get(0) : "
				 * +actualErrorMessage.get(0)); log.info(
				 * " Expected_Customer_Already_Exists_Error_Message : "
				 * +Expected_Customer_Already_Exists_Error_Message);
				 * webPage.getDriver().navigate().back(); }
				 */
			...
			// "verify_Account_Information_Tab_Verify_Validation_Messages");
			softAssert.assertAll();
			Assert.fail(e.getLocalizedMessage());
		}
	}
	 ...
	 * 
	 * @throws PageException
	 *             Verify "Change Password" checkbox functionality,Verify
	 *             "Change Password" form input validations by submitting blank
	 *             form,Verify password field validation with short password in
	 *             "New Password" field,Verify password field validation for
	...
	public void verify_Account_Information_Tab_Change_Password_Functionality() throws PageException {
		...
		// String[][] valid_data = ExcelUtil.readExcelData(DataFilePath,
		// "AccountSignINPage","VerifyChangePasswordfunctionality");
		log.info(
				"*************************************** inputdata found***************************************************");
		...
		log.info(
				" %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% Navigate_To_Account_Information_Tab_Form_URL  : ^^^^^^^^^^^^^^^^^^^^^^^^^^ "
						+ Navigate_To_Account_Information_Tab_Form_Change_Password_URL);
		String Change_Password_Link_Locator = inputdata[0][0];
		// commonMethods.clickElementbyXpath(webPage,
		// Change_Password_Link_Locator, softAssert);
		log.info(" >>>>>>>>>>>>>>>>>>>>>>........Change_Password_Link_Locator  : ^^^^^^^^^^^^^^^^^^^^^^^^^^ "
				+ Change_Password_Link_Locator);
		// commonMethods.clickElementbyXpath(webPage,
		// Change_Password_Link_Locator, softAssert);
		try {
			log.info(
			 ...
			 * +actualErrorMessage.get(0)); log.info(
			 * " *************************** inputdata[1][10]  :                      "
			 * +inputdata[1][10]); log.info(
			 * " *************************** actualErrorMessage.get(1)  :             "
			 * +actualErrorMessage.get(1)); log.info(
			 * " *************************** inputdata[2][10]  :                      "
			 * +inputdata[2][10]); log.info(
			 * " *************************** actualErrorMessage.get(2)  :             "
			 * +actualErrorMessage.get(2)); log.info(
			 * " *************************** inputdata[3][10]  :                      "
			 * +inputdata[2][10]); log.info(
			 * " *************************** actualErrorMessage.get(3)  :             "
			 * +actualErrorMessage.get(3)); log.info(
			 * " *************************** valid_data[0][10]  :                      "
			 * +inputdata[0][10]); log.info(
			 * " *************************** actualErrorMessage.get(4)  :             "
			 * +actualErrorMessage.get(4)); //log.info(
			 * " *************************** inputdata[4][10]  :                      "
			 * +valid_data[4][10]);
			 */
			...
			// softAssert.assertEquals(actualErrorMessage.get(4).trim(),
			// valid_data[1][10].trim(),"Change Password Functionality with
			// Valid Input verification failed For NA Password Input. Expected
			// Password Error Message : "+inputdata[4][10] + "Actual Email
			// Address Error Message : "+actualErrorMessage);
			softAssert.assertAll();
		} catch (Throwable e) {
			...
			softAssert.assertAll();
			/*
			 * if (brokenItems.size() > 0) { Assert.fail("Link " +
			 * Arrays.deepToString(brokenItems.toArray()) +
		...
		try {
			...
			softAssert.assertEquals(Actual_Page_Url_After_Page_Redirection, Expected_Page_URL_After_Page_Redirection,
					"Page url verification failed. Expected_Page_URL_After_Page_Redirection : "
							+ Expected_Page_URL_After_Page_Redirection + " Actual_Page_Url_After_Page_Redirection : "
			...
			softAssert.assertEquals(Actual_Page_Title_After_Page_Redirection,
					...
					"Page title verification failed. Expected_Page_Title_After_Page_Redirection : "
							+ Expected_Page_Title_After_Page_Redirection + "Actual_Page_Title_After_Page_Redirection : "
	 ...
	 * input validations by submitting blank form Application should throw an
	...
	public void verify_Contact_Information_Tab_Edit_Link_Blank_Input() throws PageException {
		...
		for (int r = 0; r < inputdata.length; r++) {
			...
			try {
				// if (FNInput.equalsIgnoreCase(" ") &&
				// LNInput.equalsIgnoreCase(" ") &&
				// EmailInput.equalsIgnoreCase(" ")) {
				List<String> actualErrorMessage = ConnsSignInPage
						.verify_Account_DashBoard_First_Name_Last_Name_Login(inputdata, softAssert);
				/*
				 * SoftAssertor.assertEquals(actualErrorMessage.get(0),
				 * Expected_Username_Password_Error_Message); log.info(
				 * "First_Name_Error_Message_Account_Information_Page : "
				 * +actualErrorMessage.get(0)); log.info("ExpectedValMsg : "
				 * +Expected_Username_Password_Error_Message);
				 * SoftAssertor.assertEquals(actualErrorMessage.get(0),
				 * Expected_Username_Password_Error_Message); log.info(
				 * "Last_Name_Error_Message_Account_Information_Page : "
				 * +actualErrorMessage.get(0)); log.info("ExpectedValMsg : "
				 * +Expected_Username_Password_Error_Message);
				 */
	 ...
	 * added under Additional Address Entries
	 * 
	 * @throws InterruptedException
	 */
			...
			if (!(FNInput.equalsIgnoreCase("NA") && LNInput.equalsIgnoreCase("NA")
				...
				List<String> information_Saved_Successfully_Message = ConnsSignInPage
						.verify_Account_Information_First_Name_Last_Name_Login_Validation_Valid_Input(inputdata,
								softAssert);
				/*
				 * String First_Name_Error_Message_Account_Information_Page =
				 * commonMethods.getTextbyXpath(webPage,FNErrLocator,
				 * softAssert); String
				 * Last_Name_Error_Message_Account_Information_Page =
				 * commonMethods.getTextbyXpath(webPage,LNErrLocator,
				 * softAssert); String
				 * Email_Address_Error_Message_Account_Information_Page =
				 * commonMethods.getTextbyXpath(webPage,EmailErrLocator,
				 * softAssert);
	 ...
	 * billing & shipping address' should be editable when tapped on Mange
	 ...
	 * address' editable form is displayed.
	 * 
	 * @throws InterruptedException
	 */
	 ...
	 * redirect to "Contact Information" form
	 * 
	 * @throws InterruptedException
	 */
	...
	public void verify_Contact_Information_Tab_Change_Billing_Address_Link()
		...
		try {
			...
			commonMethods.clickElementbyXpath(webPage, Change_Billing_Address_Edit_Link_Locator, softAssert);
			String actualPageUrl = commonMethods.getPageUrl(webPage, softAssert);
			// softAssert.assertEquals(actualPageUrl,
			// Expected_Contact_Information_Page_URL,"Page url verification
			// failed. Expected url :
	 ...
	 * should redirect to "Contact Information" form
	 * 
	 * @throws InterruptedException
	 */
	 ...
	 * https://www.conns.com/customer/address/index/
	 * 
	 * @throws InterruptedException
	 */
	...
	public void verify_Contact_Information_Tab_Go_Back_Link() throws InterruptedException {
		...
		try {
			...
			String Actual_Contact_Information_Change_Billing_Address_Page_URL = commonMethods.getPageUrl(webPage,
					softAssert);
			// softAssert.assertEquals(Actual_Contact_Information_Change_Billing_Address_Page_URL,
			// Expected_Contact_Information_Page_URL,"Page url verification
			// failed. Expected url :
			// "+Expected_Contact_Information_Page_URL+"Actual url :
			...
			// softAssert.assertEquals(actualAddressBookPageUrl,
			// Address_Book_Expected_Page_URL,"Page url verification failed.
			// Expected url : "+Address_Book_Expected_Page_URL+"Actual url :
			...
			String Actual_Contact_Information_Change_Shipping_Address_Page_URL = commonMethods.getPageUrl(webPage,
					softAssert);
			// softAssert.assertEquals(Actual_Contact_Information_Change_Shipping_Address_Page_URL,
			// Expected_Contact_Information_Page_URL,"Page url verification
			// failed. Expected url :
			// "+Expected_Contact_Information_Page_URL+"Actual url :
			// "+Actual_Contact_Information_Change_Shipping_Address_Page_URL);
	...
	public void verify_Contact_Information_Tab_Address_Book_Page_Additional_Address_Entries()
		...
		// String[][] inputdata = ExcelUtil.readExcelData(DataFilePath,
		// "AccountSignINPage","Address_Book_Manage_Addresses_Functionality");
		String Navigate_To_Account_Information_Tab_Form_URL = inputdata[0][0];
		...
		try {
			/*
			 * if( webPage.getDriver().findElement(By.xpath(inputdata[0][2])).
			 * isDisplayed())
			 */
			WebElement Address_Book_Additional_Address_Entries_Delete_Address_Locator = webPage.getDriver()
					.findElement(By.xpath(inputdata[0][2]));
			log.info("Clicked on xpath Address_Book_Additional_Address_Entries_Delete_Address_Locator"
					+ Address_Book_Additional_Address_Entries_Delete_Address_Locator.isDisplayed());
			if ((Address_Book_Additional_Address_Entries_Delete_Address_Locator).isDisplayed()) {
				 ...
				 * webPage.getDriver().switchTo().alert(); String alertText =
				 * confirmationAlert.getText(); log.info("Alert text is " +
				 * alertText); confirmationAlert.accept();
				 */
			...
			String actualAddressBookPageURL = commonMethods.getPageUrl(webPage, softAssert);
			// softAssert.assertEquals(actualAddressBookPageURL,
			// Expected_Address_Book_Page_URL,"Page url verification failed.
			// Expected url : "+Expected_Address_Book_Page_URL+"Actual url :
			// "+actualAddressBookPageURL);
			log.info(
					" **************************** Assert contains url*******************************************************************************************************************************************************");
	 ...
	 * redirect to "Newsletter subscription" form
	 * 
	 * @throws InterruptedException
	 */
	...
	public void verify_Account_Information_Tab_Newsletters_Link() throws PageException, InterruptedException {
		...
		try {
			...
			// commonMethods.clickElementbyXpath(webPage,
			// Change_Billing_Address_Edit_Link_Locator, softAssert);
			...
			softAssert.assertEquals(Actual_Newsletters_Page_URL, Expected_Newsletters_Page_URL,
					"Page url verification failed. Expected url : " + Expected_Newsletters_Page_URL + "Actual url : "
							+ Actual_Newsletters_Page_URL);
