...
public class Conns_Product_Purchase extends BaseTest {
	...
	public void Verify_Pickup_Only_Product_Add_To_Cart() {
		...
		try {
			...
			try{connsProductPurchasePage.verifyDiscountCouponCodeField(webPage, softAssert);
			}catch(Throwable t)
	...
	public void Verify_Proceed_To_Checkout_Page() {
		...
		try {
			...
			if(!(browserName.equalsIgnoreCase("Safari")||browserName.contains("iPhone")||browserName.contains("iPad"))){
				...
				/*if((browserName.equalsIgnoreCase("Safari")||browserName.contains("iPhone")||browserName.contains("iPad"))){
					//click to simulate tab event
					commonMethods.clickElementbyXpath(webPage, checkoutPageData[5][1], softAssert);
				}*/
...
//			commonMethods.clickElementbyXpath(webPage, checkoutPageData[41][1], softAssert);
	...
	public void Verify_InStockPickup_Product_Functionality() {
		...
		try {
			...
			if(connsProductPurchasePage.waitForElementToBeDisplayed(webPage,"//input[@name='shipping_method']", softAssert))
			{
				commonMethods.clickElementbyXpath(webPage,"//input[@name='shipping_method']" , softAssert);
				commonMethods.clickElementbyXpath(webPage, "//div[@id='shipping-method-buttons-container']/button", softAssert);
			}	
//			connsProductPurchasePage.waitForElementToBeDisplayed(webPage,checkoutPageData[41][1], softAssert);
	...
	public void Verify_Get_Quote_Functionality() {
		...
		try {
			...
			try{connsProductPurchasePage.verifyGetQuoteFunctionalityInvlidInput(webPage, softAssert);
			}catch(Throwable t){softAssert.fail(t.getLocalizedMessage());}
			
			try{connsProductPurchasePage.verifyGetQuoteFunctionalityValidInput(webPage, softAssert);
			}catch(Throwable t){softAssert.fail(t.getLocalizedMessage());}
	...
	public void Verify_Ship_To_Different_Address_Functionality() {
		...
		//	connsProductPurchasePage.waitIfBrowserIsIos(softAssert, 25);
			...
			try{webPage.getDriver().findElement(By.xpath(checkoutPageData[37][1])).click();
